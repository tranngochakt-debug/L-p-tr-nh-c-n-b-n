...
import { ChapterData, LessonContent } from '../data/chapter1Data.ts';
...
export const ChapterView: React.FC<ChapterViewProps> = ({
  chapterData,
  onExportMarkdown,
  exportButtonLabel = 'Tải Markdown'
}) => {
  ...
  const currentLesson: LessonContent = chapterData.lessons[selectedLessonIndex] || chapterData.lessons[0];
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="space-y-3 max-w-3xl">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-blue-200">
                Chương {chapterData.chapterId} / 10 • {chapterData.totalLessons} bài học chuẩn hóa
              </span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">
              {chapterData.chapterCode}: {chapterData.title}
            </h2>
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col gap-2 shrink-0">
            ...
            <div className="text-xs text-blue-200/80 bg-blue-950/60 border border-blue-800/50 px-3 py-2 rounded-xl text-center">
              ⏱ Tổng thời lượng: <strong>{totalMinutes} phút tự học</strong>
            </div>
          </div>
        </div>

        {/* Lesson Navigation Tabs */}
        <div className={`mt-8 pt-6 border-t border-slate-800/80 ${gridClass}`}>
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="text-xs font-semibold uppercase tracking-wider opacity-80 flex items-center gap-1.5">
                    Bài {lesson.lessonNumber} • {lesson.readingTimeMinutes}p
                  </div>
                  <div className="text-sm font-bold truncate mt-0.5" title={lesson.title}>
                    {lesson.title}
                  </div>
                </div>
              </button>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-10">
        ...
        <div className="pt-6 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="text-xs text-slate-500 font-medium">
            Bài {selectedLessonIndex + 1} / {chapterData.totalLessons} của Chương {chapterData.chapterId}
          </div>

          <button
            disabled={selectedLessonIndex === chapterData.totalLessons - 1}
            onClick={() => setSelectedLessonIndex((prev) => Math.min(chapterData.totalLessons - 1, prev + 1))}
